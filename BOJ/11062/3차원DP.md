...
public class Main {
	...
	private static int calculateHighestScore(int turn, int start, int end, int acc) {
		...
		if (turn % 2 == 0) {
			...
			if (start == end) {
				dp[start][end][1] = 0;
				return dp[start][end][0] = cards[start];
			}
			// 남아있는 카드 배열에 대해 이미 연산을 했을 때
			if (dp[start][end][0] != 0 || dp[start][end][1] != 0)
				// 둘 중 큰 값을 반환
				return dp[start][end][0];
			// 남아있는 카드 배열에 대해 아직 연산을 하지 않았을 때
			else {
				// 왼쪽 카드를 선택한 경우와 오른쪽 카드를 선택한 경우 중 최대값을 선택
				max = Math.max(cards[start] + calculateHighestScore(turn + 1, start + 1, end, acc - cards[start]),
						cards[end] + calculateHighestScore(turn + 1, start, end - 1, acc - cards[end]));
				dp[start][end][0] = max;
				dp[start][end][1] = acc - max;
				return max;
			}
		}
		// 명우 턴
			...
			// 남아있는 카드 배열에 대해 아직 연산을 하지 않았을 때
				...
				// 왼쪽 카드를 선택한 경우와 오른쪽 카드를 선택한 경우 중 최대값을 선택
				max = Math.max(cards[start] + (acc - cards[start] - left), cards[end] + (acc - cards[end] - right));
				dp[start][end][0] = max;
				dp[start][end][1] = acc - max;
				// 명우에게 우선권이 있으므로 근우의 점수로는 둘 중 작은 값을 반환
				return acc - max;
