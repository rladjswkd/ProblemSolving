### 로직 상 틀릴 게 없는데 첫 번째 시도부터 세 번째 시도까지 모두 다 틀려서 의아했는데, 타입 문제였다.

- `1 << a`와 같이 시프트 연산을 사용할 때, `a`의 타입이 `long`이든 `int`든 결괏값이 `int`로 나온다. 따라서 `long` 타입을 원할 땐 `1L << a`와 같이 시프트 연산을 수행해야 한다.
...
public class Main {
	...
	private static void solve(int chosen) {
		...
		// 방문하지 못한 비활성화 바이러스 상태인 칸들에 대한 방문 처리를 하기 위한 반복문
		for (int i = 0; i < virusCount; i++)
			visited[virusCoords[i][0]] |= 1 << virusCoords[i][1];
