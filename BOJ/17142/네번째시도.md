- `masks`와 `visited`배열은 방문 확인에만 사용하고, 모든 빈 칸을 다 방문했는지는 초반에 `lab`정보를 입력받을 때 빈 칸의 개수를 세고 BFS에서 빈 칸을 방문할 때마다 1씩 빼, 최종적으로 그 값이 0인지 확인하는 식으로 하면 더 빠르게 처리할 수 있을 것이다.
